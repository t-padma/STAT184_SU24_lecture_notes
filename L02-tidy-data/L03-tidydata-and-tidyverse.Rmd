...
* In the example above, we want to check if class average _depends_ on the instructor's experience. Hence "class average" is the dependent variable while "Class No." and "Instructor experience" are independent variables. 
...
* Each factor level is called a **treatment**. In other words, a treatment (instructor experience) is assigned to each of the six classes.
...
* Is the data messy? If yes, what does the tidy version look like?
...
# Where will you take all the colnames and values to is specified by names_to and values_to
```
